...
{
	const Position& targetPos = target->getPosition();
	int32_t lifeLeech = 0, manaLeech = 0, lifeAbsorb = 0, manaAbsorb = 0;
	if(healthChange > 0)
	{
	...
	else
	{
		...
		if(damage > 0)
		{
			...
			if(damage > 0)
			{
				...
				Player* player = NULL;
				if(attacker && (player = attacker->getPlayer())){
					if(random_range(1,100) < player->getCriticalHitChance()){
						damage *= 2;
						addAnimatedText(player->getPosition(), COLOR_DARKRED, "Critical!");
					}	
				}
				
				if((player = target->getPlayer()) && (attacker != target) && target == player){
					if(random_range(1,100) < player->getDodgeChance())
					{
						damage *= 0.5;
						addAnimatedText(player->getPosition(), COLOR_BLUE, "Dodge!");
					}
				}
				...
				if(textColor < COLOR_NONE && magicEffect < MAGIC_EFFECT_NONE)
				{
					...
					// aqui - ver se não funcionar add o Details

					if(attacker && (player = attacker->getPlayer())) {
						if(player->getLifeLeech() > 0) {
							lifeLeech = damage * (player->getLifeLeech() * 0.01);
							if(lifeLeech > 0) {
								combatChangeHealth(params, target, attacker, lifeLeech, true);
							}
						}
						
						if(player->getManaLeech() > 0) {
							manaLeech = damage * (player->getManaLeech() * 0.01);
							if(manaLeech > 0) {
...
// Absorb
					if((player = target->getPlayer()) && (attacker != target)) {
						...
						if(player->getManaAbsorb() > 0)	{
							manaAbsorb = damage * (player->getManaAbsorb() * 0.01);
							if(manaAbsorb > 0)
							{
	...
	CombatType_t combatType/* = COMBAT_MANADRAIN*/, bool inherited/* = false*/)
{
	...
	else
	{
		...
		if(manaLoss > 0)
		{
			...
			if(attacker && (player = attacker->getPlayer())) {
				if(player->getLifeLeech() > 0)	{
					lifeLeech = manaLoss * (player->getLifeLeech() * 0.01);
					if(lifeLeech > 0) {
				...
				if(player->getManaLeech() > 0) {
					manaLeech = manaLoss * (player->getManaLeech() * 0.01);
					if(manaLeech > 0) {
			...
			if((player = target->getPlayer()) && (attacker != target)) {
				if(player->getLifeAbsorb() > 0) {
					lifeAbsorb = manaLoss * (player->getLifeAbsorb() * 0.01);
					if(lifeAbsorb > 0) {
				...
				if(player->getManaAbsorb() > 0)	{
					manaAbsorb = manaLoss * (player->getManaAbsorb() * 0.01);
					if(manaAbsorb > 0) {
...
void Game::showHotkeyUseMessage(Player* player, Item* item)
{
	...
	if(!it.showCount)
		stream << "Consumindo um " << it.name << "...";
	else if(count == 1)
		stream << "Consumindo ultimo " << it.name.c_str() << "...";
	else
		stream << "Consumindo um " << count << " " << it.pluralName.c_str() << "...";
